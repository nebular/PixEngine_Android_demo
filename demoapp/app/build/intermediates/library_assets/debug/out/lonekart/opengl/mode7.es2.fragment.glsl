...
const float M7Params_height_perspective =  -3.14159265 / 3.0;
...
vec4 map(sampler2D bitmap, vec2 fragCoord, bool inverse, bool heights) {
    ...
    for (int h=0; h<int(MAXHEIGHT); h++) {

        float hs = MAXHEIGHT - float(h);// hs: +32 +31 +30 +29 ... NORM => 1...0 down...up
...
vec4 drawImage(vec4 spriteSheet, vec2 uv, vec4 imageVector, sampler2D source, vec4 spriteDef, vec4 tintInfo, vec4 groundPix, bool blur, bool useHeight) {
    ...
    if (hasImage(spriteIndex)) {
        ...
        vec2 sprite = vec2(mod(float(spriteIndex),float(spriteSheet.z)), spriteIndex/int(spriteSheet.z));
...
vec4 mainSprites(vec4 face, bool useHeights) {
    ...
    if (iSpriteSheets > 0 && iSpriteSheets < 9)
    ...
    for (int i=0; i<MAXSPRITES; i++) {
        ...
        if (iSpriteDef0[i][0]>=0.)                    face = mixSprite(iSpriteSheet0, fragCoord, iSpritePos0[i], spritebuffer0, face, iSpriteDef0[i], iSpriteFx0[i], BLUR, useHeights);
/*
        ...
        if (iSpriteSheets>3 && iSpriteDef3[i][0]>=0.) face = mixSprite(iSpriteSheet3, fragCoord, iSpritePos3[i], spritebuffer3, face, iSpriteDef3[i], iSpriteFx3[i], BLUR, useHeights);
*/
        if (iSpriteSheets>4 && iSpriteDef4[i][0]>=0.) face = mixSprite(iSpriteSheet4, fragCoord, iSpritePos4[i], spritebuffer4, face, iSpriteDef4[i], iSpriteFx4[i], BLUR, useHeights);
        if (iSpriteSheets>5 && iSpriteDef5[i][0]>=0.) face = mixSprite(iSpriteSheet5, fragCoord, iSpritePos5[i], spritebuffer5, face, iSpriteDef5[i], iSpriteFx5[i], BLUR, useHeights);
        if (iSpriteSheets>6 && iSpriteDef6[i][0]>=0.) face = mixSprite(iSpriteSheet6, fragCoord, iSpritePos6[i], spritebuffer6, face, iSpriteDef6[i], iSpriteFx6[i], BLUR, useHeights);
        if (iSpriteSheets>7 && iSpriteDef7[i][0]>=0.) face = mixSprite(iSpriteSheet7, fragCoord, iSpritePos7[i], spritebuffer7, face, iSpriteDef7[i], iSpriteFx7[i], BLUR, useHeights);
...
void main() {
    ...
    if (face.w == 1. ) {
      ...
      mapColor = mainSprites(mapColor, TexCoord.y>0.5);
        gl_FragColor = mapColor;// ignore alpha on UI
