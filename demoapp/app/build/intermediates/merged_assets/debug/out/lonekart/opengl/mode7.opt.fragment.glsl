...
const float M7Params_space_z = 55.0;
const float M7Params_horizon = 20.0;
const vec2  M7Params_scale = vec2(200,200);
const float M7Params_height_perspective =  -3.14159265 / 3.;
const float M7Params_sinPerspective = sin(M7Params_height_perspective);
...
float MAXHEIGHT = abs(M7Params_space_z * M7Params_sinPerspective);
...
vec2 getSample(vec2 fragCoord) {
    ...
    float cosCam = cos(iCamWorld.z), sinCam = sin(iCamWorld.z);
    // calculate the dx and dy of points in space when we step
    // through all points on this line
    vec2 linedelta = vec2(
    -sinCam* horizontal_scale,
    cosCam * horizontal_scale
    );

    // calculate the starting position
    vec2 space = vec2(
    iCamWorld.x*iMapSize.x +  distance * cosCam - iResolution.x/2. * linedelta.x,
    iCamWorld.y*iMapSize.y +  distance * sinCam - iResolution.x/2. * linedelta.y
    );
...
vec4 map(sampler2D bitmap, vec2 fragCoord, bool inverse, bool heights) {
    ...
    fragCoord.y = inverse ?  iResolution.y/4. - fragCoord.y : fragCoord.y;
    ...
    for (float h=0.; h<MAXHEIGHT; h++) {
        ...
        if (newCoord.y > 0.) {
            ...
            float newhoffs =  M7Params_sinPerspective * newHeight * M7Params_space_z  * (1. - (distance*0.5));
            ...
            if (newHeight != 0. && newhoffs<=-hs) {
                ...
                return heights
                ? vec4(mixtrail(groundpix*kg, newSample).xyz, groundpix.w)
                : mixtrail(groundpix*kg, newSample);
            }
        }
    }
    ...
    if (fragCoord.y<0.) return vec4(-1.);
...
vec4 drawImage(vec4 spriteSheet, vec2 uv, vec4 imageVector, sampler2D source, vec4 spriteDef, vec4 tintInfo, vec4 groundPix, bool blur, bool useHeight) {
    ...
    if (hasImage(spriteIndex)) {
        ...
        vec2 sprite = vec2(mod(float(spriteIndex), spriteSheet.z), spriteIndex/int(spriteSheet.z));

        float t = 0.0;

        vec4 fincolor = VEC4Z;
        vec2 pos = iResolution.xy * imageVector.xy;
        vec2 dimens = __spriteSize * imageVector.z * spriteDef.zw;
        ...
        && (uv.y > pos.y - halfdimens.y) && (uv.y < pos.y +  halfdimens.y)) {
            ...
            if (imageVector.w != 0.) {
                ...
                textpos -= halfdimens;
                textpos = rotate2d(imageVector.w) * textpos;
...
vec4 getSpritePix(in vec4 spritesheet, in vec2 fragCoord, in vec4 imageVector, in sampler2D channel, in vec4 destinationLayer, in vec4 spriteDef, in vec4 tintInfo, bool blur, bool useHeights) {
    return drawImage(spritesheet, fragCoord, imageVector, channel, spriteDef, tintInfo, destinationLayer, blur, useHeights);
}
...
vec4 mainSprites(vec4 face, bool useHeights) {
    ...
    vec4 zzzz = vec4(0,0,0,0);

    for (int i=0; i<MAXSPRITES; i++) {

        // sprite active / inactive
        vec4 def14 = vec4(iSpriteDef0[i][0], iSpriteDef1[i][0], iSpriteDef2[i][0], iSpriteDef3[i][0]);
        // zeros if no sprite, ones if sprite
        vec4 sss = when_neq(def14, vec4(-1));
        // zeros if no active spritesheet, ones if active spritesheet & active sprite
        vec4 spact = when_gt(vec4(iSpriteSheets), vec4(0,1,2,3)) * sss;

        // first four spritesheets
        face = spact.x == 1. ? mixSprite(iSpriteSheet0, fragCoord, iSpritePos0[i], spritebuffer0, face, iSpriteDef0[i],iSpriteFx0[i], BLUR, useHeights) : face;
        face = spact.y == 1. ? mixSprite(iSpriteSheet1, fragCoord, iSpritePos1[i], spritebuffer1, face, iSpriteDef1[i],iSpriteFx1[i], BLUR, useHeights) : face;
        face = spact.z == 1. ? mixSprite(iSpriteSheet2, fragCoord, iSpritePos2[i], spritebuffer2, face, iSpriteDef2[i],iSpriteFx2[i], BLUR, useHeights) : face;
        face = spact.a == 1. ? mixSprite(iSpriteSheet3, fragCoord, iSpritePos3[i], spritebuffer3, face, iSpriteDef3[i],iSpriteFx3[i], BLUR, useHeights) : face;

        // sprite active / inactive
        vec4 def48 = vec4(iSpriteDef4[i][0], iSpriteDef5[i][0], iSpriteDef6[i][0], iSpriteDef7[i][0]);
        // zeros if no sprite, ones if sprite
        sss = when_neq(def48, vec4(-1));
        // zeros if no active spritesheet, ones if active spritesheet & active sprite
        spact = when_gt(vec4(iSpriteSheets), vec4(4,5,6,7)) * sss;

        // last four spritesheets
        //		face = spact.x == 1 ? mixSprite(iSpriteSheet4, fragCoord, iSpritePos4[i], spritebuffer4, face, iSpriteDef4[i],iSpriteFx4[i], BLUR, useHeights) : face;
        //		face = spact.y == 1 ? mixSprite(iSpriteSheet5, fragCoord, iSpritePos5[i], spritebuffer5, face, iSpriteDef5[i],iSpriteFx5[i], BLUR, useHeights) : face;
        //		face = spact.z == 1 ? mixSprite(iSpriteSheet6, fragCoord, iSpritePos6[i], spritebuffer6, face, iSpriteDef6[i],iSpriteFx6[i], BLUR, useHeights) : face;
        //		face = spact.a == 1 ? mixSprite(iSpriteSheet7, fragCoord, iSpritePos7[i], spritebuffer7, face, iSpriteDef7[i],iSpriteFx7[i], BLUR, useHeights) : face;
...
vec4 main_groundTexture(vec4 skypix) {
    ...
    return grd != vec4(-1.) ? grd : skypix;
    // color comes also with height information in w

    //	if (grd != vec4(-1,-1,-1,-1)) mapColor = grd;
    // color comes also with height information in w

    // }
}

vec4 main_sky_parallax_far(vec2 fragCoord) {
    ...
    return (hor.w!=0. && (hor.x!=0. || hor.y!=0. || hor.z!=0.))
    // apply a fading consstant to thr far parallax pixel
    ? hor * paxcolorfade
    : map(m7clouds, fragCoord, true, false);

}

vec4 main_sky_parallax_near(vec2 fragCoord) {
    float sc=PARALLAX_FAR_SCALE;					// far parallax layer scale
    ...
    return (hor.w!=0. && (hor.x!=0. || hor.y!=0. || hor.z!=0.))
    ? hor
    // else if We are in the far parallax layer
    : (origCoord.y > 0.25 - 0.25 / sc) ? main_sky_parallax_far(fragCoord)
    // else plain sky
    : map(m7clouds, fragCoord, true, false);
}

vec4 main_skyTexture() {
    // sky texture coordinates
    vec2 fragCoord = TexCoord*vec2(iResolution.x, iResolution.y/2.);
    return TexCoord.y > 0.25
    ? main_sky_parallax_near(fragCoord)
    // else plain sky
    : map(m7clouds, fragCoord, true, false);
}
...
vec4 main_groundOrSky() {

    vec4 mapColor = vecOnes;

    bool isSky = TexCoord.y < 0.5;
    bool isGround = TexCoord.y >= 0.5-(MAXHEIGHT/iResolution.y/2.);

    mapColor = isSky
    ? isGround ? main_groundTexture(main_skyTexture()) : main_skyTexture()
    : isGround ? main_groundTexture(mapColor) : mapColor;

    return invertGround ? vec4(vec3(1.)-mapColor.xyz, mapColor.w) : mapColor;
}

void main() {

    yuvColorKey = yuv(iColorKey);
    invertGround = iCamWorld.w == 1.;// FX are sent in iCamWorld. Currently w==1 => invert
    ...
    FragColor = mainSprites(main_groundOrSky(), TexCoord.y>0.5);
